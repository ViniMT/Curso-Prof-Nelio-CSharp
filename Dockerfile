...
WORKDIR /src
COPY ["POO.csproj", "."]
RUN dotnet restore "./POO.csproj"
COPY . .
WORKDIR "/src/."
RUN dotnet build "./POO.csproj" -c %BUILD_CONFIGURATION% -o /app/build

# Esta fase é usada para publicar o projeto de serviço a ser copiado para a fase final
FROM build AS publish
ARG BUILD_CONFIGURATION=Release
RUN dotnet publish "./POO.csproj" -c %BUILD_CONFIGURATION% -o /app/publish /p:UseAppHost=false
